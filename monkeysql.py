...
def init(db_url=None, **params):
    global db
    db = web.database(db_url, **params)
...
def select(model, **kwargs):
    ...
    """
    result = []
    for dbobj in db.select(model.__table_name__, **kwargs):
...
def multiple_insert(model, values):
    """
        ...
        print users[1].id
    """
    out = db.multiple_insert(model.__table_name__, [value._getinsertrow() for value in values])
...
def multiple_delete(model, values):
    ...
    """
    db.delete(model.__table_name__, where="id in (%s)" % ",".join([value.id for value in values]))
...
class Model(object):
    ...
    def save(self):
        ...
        if "id" not in self.__dict__:
            self.id = db.insert(self.__table_name__, **self._getinsertrow())
        elif len(self._dirty) > 0:
            db.update(self.__table_name__, where=('id=%d' % self.id), **self._get_update_row())
            self._dirty = []
    ...
    def delete(self):
        ...
        if self.id is not None:
            db.delete(self.__table_name__, where=('id=%d' % self.id))
